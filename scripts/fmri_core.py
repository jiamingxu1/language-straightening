...
# Get SNR for a given ROI (input should be ROI-averaged data)
...
def build_roi_voxs(
    ...
    # ---------- functional ROIs ----------
    xfm = get_xfm(subject)
    mask3d = load_mask(subject)

    f_roi_voxs = {}

    for roi in f_rois:
        try:
            one = cortex.utils.get_roi_masks(
                subject,
                xfm,
                roi_list=[roi],          
                gm_sampler="cortical",
            ...
            # ROI truly not present for this subject
            continue

        roi_mask3d = one[roi]
        idx = np.flatnonzero(roi_mask3d[mask3d])
